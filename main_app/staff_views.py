...
@csrf_exempt
def get_students_for_noc(request):
    ...
    for student in students:
        html += f'<option value="{student.id}">{student.admin.last_name}, {student.admin.first_name}</option>'
    
    return HttpResponse(html)

def student_noc_details(request, student_id):
    staff = get_object_or_404(Staff, admin=request.user)
    try:
        student = Student.objects.get(id=student_id)
    except Student.DoesNotExist:
        messages.error(request, "No student found with the given ID.")
        return redirect('staff_sign_noc')
    ...
    context = {
        ...
        'subjects': [
            {
                'subject': subject,
                'attendance': calculate_attendance(student, subject),
    ...
    return render(request, 'staff_template/student_noc_details.html', context)

def calculate_attendance(student, subject):
    total_classes = Attendance.objects.filter(subject=subject).count()
    attended_classes = AttendanceReport.objects.filter(student=student, attendance__subject=subject, status=True).count()
    if total_classes > 0:
        attendance_percentage = (attended_classes / total_classes) * 100
        return f"{attended_classes}/{total_classes} ({attendance_percentage:.2f}%)"
    return "0/0 (0.00%)"
...
def get_assignment_status(request):
    if request.method == 'POST':
        ...
        try:
            submission = AssignmentSubmission.objects.get(student_id=student_id, subject_id=subject_id)
...
def SIGN_NOC(request, student_id, subject_id):
